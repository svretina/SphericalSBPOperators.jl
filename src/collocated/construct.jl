...
function _rows_with_nonzero_first_column(
        G::SparseMatrixCSC{
            T, Ti,
        }; atol::T
    ) where {T <: Real, Ti <: Integer}
...
function _infer_boundary_accuracy_order(
    ...
    boundary_accuracy = -1
    row = 1
    for degree in 0:interior_accuracy
        u = xfull .^ degree
...
function _resolve_stencil_cols(
...
"""
function compute_even_gradient_row_decoupled(
        row_index::Int,
...
"""
function verify_even_gradient_row_decoupled(
        row_index::Int,
        stencil_indices::Vector{Int},
...
function _solve_exact_linear_system(
...
function _verify_coupled_origin_repair(
...
function _solve_coupled_geven_block(
        Geven::SparseMatrixCSC{T, Ti},
        rows_to_solve::Vector{Int},
    ...
    all(Base.Fix2(in, stencil_set), rows_to_solve) ||
    ...
    for i in rows_to_solve
        ...
        hi != 0 // 1 ||
            throw(ArgumentError("Encountered zero mass diagonal at i=$i in coupled divergence constraints."))
...
function _repair_even_gradient_first_column!(
        Geven::SparseMatrixCSC{T, Ti},
        Dfull,
        Gfull::SparseMatrixCSC{T, Ti},
        xfull::Vector{T},
    ...
    if isnothing(custom_stencil_cols)
        ...
        while true
            ...
            for len_try in start_len:(Nh - 1)
                ...
                result = _solve_coupled_geven_block(
                    Geven,
                    rows_to_solve,
                    stencil_try,
                    num_constraints,
                ...
                x_try, map_try, n_eq_try, n_unk_try = result
                x = x_try
                var_index = map_try
        ...
        result === nothing &&
            throw(
            ArgumentError(
                "Coupled row/column constraints are inconsistent; no exact solution exists for this stencil."
            )
        )
        x, var_index, n_equations, n_unknowns = result
    end
...
function _assert_global_even_interior_accuracy(
        Geven::SparseMatrixCSC{T, Ti},
        r::Vector{T},
        interior_accuracy::Int,
        rows_to_solve::Vector{Int},
    ...
    if T <: AbstractFloat
        err = maximum(
            abs.(
                view(dphi_num, start_idx:stop_idx) .-
