...
function _sat_boundary_coefficients(BNN::Real, HNN::Real; bc::Symbol)
    ...
    HNN == 0 &&
        throw(ArgumentError("`H[end,end]` must be nonzero when SAT boundary terms are enabled."))
...
constraint (`dPsi[1]=0`).
"""
function assemble_block_operator(
        D::AbstractMatrix,
        G::AbstractMatrix;
        H = nothing,
        B = nothing,
    ...
    if bc_norm !== :none
        H === nothing &&
            throw(ArgumentError("Pass `H` when SAT boundary terms are enabled."))
        B === nothing &&
            throw(ArgumentError("Pass `B` when SAT boundary terms are enabled."))
...
"""
function rk4_dt_max_from_spectrum(eigvals::AbstractVector{<:Complex}; tol::Float64 = 1.0e-10)
    ...
    if hi >= 1.0e9 && rk4_amplification_max(eigvals, hi) <= 1 + tol
        return hi
    end
...
function spectral_analysis(
    ...
    spectral_radius_mode_eigenvalue = spectral_radius_mode_index == 0 ? (0.0 + 0.0im) :
        eigvals[spectral_radius_mode_index]
    ...
    if throw_on_instability && !isempty(unstable_indices)
        throw(
            ArgumentError(
                "Exponential instability detected: max Re(lambda) = $(max_real), unstable mode count = $(length(unstable_indices)).",
            )
        )
    end
...
function test_pole_stability(
    ...
    nD1 == nG1 == nH1 ||
        throw(DimensionMismatch("`D`, `G`, and `H` must have identical sizes."))
    ...
    @printf(
        "Pole stability test (bc=%s, enforce_origin=%s)\n", String(bc_norm),
        string(enforce_origin)
    )
    @printf("  max Re(lambda) = %.12e (tol = %.3e)\n", max_real, tol)
    @printf(
        "  unstable eigenvalues (Re > tol): %d / %d\n", unstable_count,
        length(eigvals_closed)
    )
    println(
        "  verdict: ",
        is_stable ? "stable (no growing modes above tolerance)" :
            "unstable (growing modes detected)"
    )

    if make_plot
        path = isnothing(spectrum_plot_path) ?
            joinpath(
                "plots", "stability", "pole_closed_spectrum_" * String(bc_norm) *
                ".png"
            ) :
...
function _build_even_folding_maps(xfull::AbstractVector{<:Real}; atol::Float64 = 1.0e-12)
    xf = Float64.(xfull)
    m = length(xf)
    sorted_full_idx = sortperm(xf)
    half_indices = [i for i in sorted_full_idx if xf[i] >= -atol]
    isempty(half_indices) &&
        throw(ArgumentError("Could not identify nonnegative half-grid from full Cartesian grid."))

    r = xf[half_indices]
    abs(r[1]) <= atol ||
        throw(ArgumentError("Folded half-grid does not start at the origin."))
...
function _as_dense_square_matrix(op, n::Int)
    if applicable(Matrix, op)
        maybe_dense = Matrix(op)
        if size(maybe_dense) == (n, n)
...
function build_folded_sbp_dissipation(ops; atol::Float64 = 1.0e-12)
    n = size(getproperty(ops, :H), 1)
    n >= 2 ||
        throw(ArgumentError("Need at least two half-grid nodes to build dissipation."))

    m_full = hasproperty(ops, :M_full) ? Int(getproperty(ops, :M_full)) : (2 * (n - 1) + 1)
    m_full > 0 ||
        throw(ArgumentError("Invalid full-grid size inferred for base dissipation operator."))
...
"""
function test_pole_stability_dissipative_overlay(
        ops;
    ...
    n = size(Df, 1)
    size(Gf, 1) == n == size(Gf, 2) ||
        throw(DimensionMismatch("`ops.Geven` must be $n x $n."))
    ...
    if throw_on_violation
        is_stable ||
            throw(
            ArgumentError(
                "Dissipative operator unstable: max Re(lambda) = $max_real_diss > tol = $tol.",
            )
        )
        shifted_left ||
            throw(
            ArgumentError(
                "Dissipative spectrum is not shifted left enough: baseline=$max_real_closed, diss=$max_real_diss, tol=$tol.",
            )
        )
    end
...
function zero_crossings(u::AbstractVector{<:Real}; atol::Float64 = 1.0e-12)
    n = length(u)
    n <= 1 && return 0
...
function oscillation_metrics(u::AbstractVector{<:Real}; atol::Float64 = 1.0e-12)
    ...
    return (
        zero_crossings = zc, zc_ratio = zc_ratio, total_variation = tv, tv_ratio = tv_ratio,
    )
end
...
"""
function nullspace_and_checkerboard(
    ...
    for eig_idx in null_indices
        ...
        pi_checker = (pi_metrics.zc_ratio >= checkerboard_zc_ratio) &&
            (pi_metrics.tv_ratio >= checkerboard_tv_ratio)
        psi_checker = (psi_metrics.zc_ratio >= checkerboard_zc_ratio) &&
            (psi_metrics.tv_ratio >= checkerboard_tv_ratio)
...
function inward_packet_initial_data(
    ...
    k = isnothing(wavenumber) ? 0.35 * (pi / max(dr, 1.0e-12)) : Float64(wavenumber)
...
function reflection_ibvp_test(
    ...
    sat = if bc_norm === :none
        ...
        B === nothing &&
            throw(ArgumentError("Pass `B` when `boundary_condition != :none`."))
        Bf = Matrix{Float64}(B)
    ...
    nsteps <= max_steps ||
        throw(ArgumentError("Reflection test requires $nsteps steps (> max_steps=$max_steps). Increase `dt`, decrease `t_final`, or increase `max_steps`."))
    dt_use = t_end / nsteps
    ...
    sol = solve(
        prob, RK4(); adaptive = false, dt = dt_use, saveat = saveat, verbose = false
    )
...
function save_spectrum_overlay_plot(
        eigvals_baseline::AbstractVector{<:Complex},
...
function save_nullspace_plot(null_report, path::AbstractString; max_modes::Int = 6)
    if isempty(null_report.entries)
        ...
        text!(
            ax, 0.5, 0.5, text = L"\mathrm{No\ near\text{-}zero\ modes\ detected}",
    ...
    for k in 1:nm
        ...
        mode_title = latexstring(
            "\\mathrm{mode}\\ ", entry.eigen_index,
            "\\;|\\;\\mathrm{spurious}=",
        ...
        lines!(
            ax, entry.psi_mode; color = :darkorange,
            linewidth = 2, linestyle = :dash, label = L"\Psi"
        )
        axislegend(ax; position = :rt)
    end
...
function save_dashboard(report, path::AbstractString)
    ...
    scatter!(
        ax1, real.(report.spectral.eigvals), imag.(report.spectral.eigvals);
    ...
    ax2 = Axis(
        fig[1, 2], xlabel = L"i_r", ylabel = L"\mathrm{mode\ amplitude}",
        title = L"\mathrm{Null\ Space\ Modes}"
    )
    if isempty(report.nullspace.entries)
        text!(
            ax2, 0.5, 0.5, text = L"\mathrm{No\ near\text{-}zero\ modes}",
            space = :relative, align = (:center, :center)
        )
    else
        ...
        lines!(
            ax2, mode.psi_mode; color = :darkorange,
            linewidth = 2, linestyle = :dash, label = L"\Psi"
        )
    ...
    lines!(
        ax4, report.reflection.t, report.reflection.energy; color = :black, linewidth = 2
    )
...
function print_suite_summary(report)
    ...
    @printf(
        "boundary condition in spectrum: %s\n",
    ...
    return if !isempty(report.plots)
        plot_pair_sort_key(plot_pair) = string(plot_pair[1])
        @printf("plots written to:\n")
        for (k, v) in sort(collect(report.plots); by = plot_pair_sort_key)
            @printf("  %-12s %s\n", string(k) * ":", v)
        end
    end
end
