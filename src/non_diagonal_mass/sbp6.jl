"""
    sbp6_v_offdiag_pairs(N; outer_boundary_closure_help=false)

Return the ordered list of free off-diagonal index pairs `(i,j)` used by the
publication non-diagonal SBP6 vector-mass pattern.

Pattern:
- left/origin closure: `(2,3)`, `(2,4)`, `(3,4)`, `(3,5)`, `(4,5)`, `(5,6)`, `(6,7)`
- optional right/outer closure help: `(N-2,N-1)`, `(N-1,N)`
"""
function sbp6_v_offdiag_pairs(N::Integer; outer_boundary_closure_help::Bool = false)
    Nint = Int(N)
    Nint >= 17 ||
        throw(ArgumentError("SBP6 requires `N >= 17` to keep the fixed left/right closures disjoint."))
...
@inline _sbp6_left_diag_indices(N::Int) = collect(2:min(10, N))
@inline _sbp6_interior_diag_indices(N::Int; outer_boundary_closure_help::Bool = false) =
    outer_boundary_closure_help ? (11 <= N - 7 ? collect(11:(N - 7)) : Int[]) :
    (11 <= N ? collect(11:N) : Int[])
@inline _sbp6_right_diag_indices(N::Int; outer_boundary_closure_help::Bool = false) =
    outer_boundary_closure_help ? collect((N - 6):N) : Int[]

"""
    sbp6_vector_mass(; N=17, v_symbol_prefix="v", s_symbol_prefix="s",
                         outer_boundary_closure_help=false)

Construct the symbolic SBP6 vector-mass template used in the paper.
"""
function sbp6_vector_mass(;
                              N::Integer = 17,
                              v_symbol_prefix::AbstractString = "v",
                              s_symbol_prefix::AbstractString = "s",
                              outer_boundary_closure_help::Bool = false)
    Nint = Int(N)
    Nint >= 17 ||
        throw(ArgumentError("SBP6 requires `N >= 17`."))
    ...
    diag_free_indices = vcat(_sbp6_left_diag_indices(Nint),
                             _sbp6_right_diag_indices(Nint;
    ...
    @inbounds for i in _sbp6_interior_diag_indices(Nint;
                                                       outer_boundary_closure_help = outer_boundary_closure_help)
    ...
    return (S = S,
            ...
            left_diag_indices = _sbp6_left_diag_indices(Nint),
            interior_diag_indices = _sbp6_interior_diag_indices(Nint;
                                                                     outer_boundary_closure_help = outer_boundary_closure_help),
            right_diag_indices = _sbp6_right_diag_indices(Nint;
                                                               outer_boundary_closure_help = outer_boundary_closure_help),
            outer_boundary_closure_help = outer_boundary_closure_help)
end
...
Construct the publication SBP6 vector-mass matrix `V` from scalar diagonal entries
`s_diag`.
...
function sbp6_vector_mass(s_diag::AbstractVector{T};
                              ...
                              outer_boundary_closure_help::Bool = false) where {
                                                                                               T <:
                                                                                               Real}
    N = length(s_diag)
    N >= 17 ||
        throw(ArgumentError("SBP6 requires `length(s_diag) >= 17`."))
    ...
    diag_free_indices = vcat(_sbp6_left_diag_indices(N),
                             _sbp6_right_diag_indices(N;
                                                          outer_boundary_closure_help = outer_boundary_closure_help))
    ...
    pairs = sbp6_v_offdiag_pairs(N; outer_boundary_closure_help = outer_boundary_closure_help)
...
delegates to `sbp6_vector_mass(sdiag; kwargs...)`.
"""
...
function _sbp6_constraint_rows(N::Int,
                                   first_rows_r5::Int,
                                   closure_right::Int;
                                   outer_boundary_closure_help::Bool = false)
...
"""
    sbp6_solve_accuracy_constraints(setup; first_rows_r5=3,
                                        exact_solve=true, verbose=true)

Solve the publication non-diagonal SBP6 linear system using:
...
function sbp6_solve_accuracy_constraints(setup::NamedTuple;
    ...
    Tq = Rational{BigInt}
    rq = Tq[_as_big_rational(ri) for ri in r]
    I, J, GV = findnz(sparse(Geven))
    Gq = sparse(I, J, Tq[_as_big_rational(gij) for gij in GV], N, N)
    Rq = _as_big_rational(setup.R)
    ...
    residual_exact = m == 0 ? 0 // 1 : _maxabs(abs.(A * x_exact .- bvec))
    ...
    S = spdiagm(0 => sdiag)
    V = sbp6_vector_mass(sdiag;
                             v11 = one(Tm),
                             v_diag_block = v_diag_block,
                             v_offdiag_block = v_offdiag_block,
                             outer_boundary_closure_help = outer_boundary_closure_help)
    div_data = sbp6_construct_divergence(S,
                                             V,
                                             G_use,
                                             r_use;
                                             p = p)
    ...
    err_r = _constraint_error(D, r_use, p, 1, rows.rows_r)
    err_r3 = _constraint_error(D, r_use, p, 3, rows.rows_r3)
    err_r5 = _constraint_error(D, r_use, p, 5, rows.rows_r5)
    quad_val = sum(sdiag)
    quad_target = Rq^(p + 1) / convert(Tq, p + 1)
    quad_err = Float64(abs(quad_val - quad_target))

    s_sym = _is_symmetric(S)
    v_sym = _is_symmetric(V)
    s_pd = all(si -> si > 0 // 1, sdiag)
    v_pd = _is_positive_definite(V)

    if verbose
        println("SBP6 publication solve mode: exact")
...
then solves the publication split-mass constraints.
"""
function sbp6_solve_accuracy_constraints(source;
                                             ...
                                             exact_solve::Bool = true,
                                             verbose::Bool = true)
    setup = sbp6_scalar_mass_gradient(source;
    ...
    return sbp6_solve_accuracy_constraints(setup;
...
Build publication non-diagonal-mass SBP6 operators by solving the split-mass
...
# Compatibility aliases for the historical publication implementation name.
@inline sbp6_exp_v_offdiag_pairs(args...; kwargs...) = sbp6_v_offdiag_pairs(args...; kwargs...)
@inline sbp6_exp_vector_mass(args...; kwargs...) = sbp6_vector_mass(args...; kwargs...)
@inline sbp6_exp_scalar_mass_gradient(args...; kwargs...) = sbp6_scalar_mass_gradient(args...; kwargs...)
@inline sbp6_exp_construct_divergence(args...; kwargs...) = sbp6_construct_divergence(args...; kwargs...)
@inline sbp6_exp_solve_accuracy_constraints(args...; kwargs...) = sbp6_solve_accuracy_constraints(args...; kwargs...)
@inline sbp6_exp_operators(args...; kwargs...) = sbp6_operators(args...; kwargs...)
