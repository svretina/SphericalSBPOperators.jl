"""
    sbp6_experimental_v_offdiag_pairs(N)
...
- row 6: `(6,7)`
"""
function sbp6_experimental_v_offdiag_pairs(N::Integer)
    Nint = Int(N)
...
"""
    sbp6_experimental_vector_mass(; N=10, v_symbol_prefix="v", s_symbol_prefix="s")
...
- `v_offdiag_symbols`: off-diagonal unknowns in `sbp6_experimental_v_offdiag_pairs(N)` order,
- `v_offdiag_pairs`: off-diagonal pair list,
- `s_tail_symbols`: tail symbols `[s_11, ..., s_N]` reused on `V` diagonal entries
  where `V[i,i] = S[i,i]`.
"""
function sbp6_experimental_vector_mass(; N::Integer=10,
    ...
    v_offdiag_pairs = sbp6_experimental_v_offdiag_pairs(Nint)
...
function _sbp6_experimental_extract_diagonal(S::SparseMatrixCSC{T,Ti}) where {T<:Real,Ti<:Integer}
    n, m = size(S)
...
"""
    sbp6_experimental_vector_mass(s_diag; v11=one(eltype(s_diag)),
...
- off-diagonals are only on `sbp6_experimental_v_offdiag_pairs(N)` and are set by
  `v_offdiag_block` (defaults to zeros)

The order of `v_diag_block` is `[v[2], v[3], ..., v[min(10,N)]]`.
The order of `v_offdiag_block` matches `sbp6_experimental_v_offdiag_pairs(N)`.
"""
function sbp6_experimental_vector_mass(s_diag::AbstractVector{T};
    v11=one(T),
    ...
    pairs = sbp6_experimental_v_offdiag_pairs(N)
...
"""
    sbp6_experimental_vector_mass(S::SparseMatrixCSC; kwargs...)

Convenience overload that extracts `s_diag` from a diagonal scalar-mass matrix `S`
and delegates to `sbp6_experimental_vector_mass(s_diag; kwargs...)`.
"""
function sbp6_experimental_vector_mass(S::SparseMatrixCSC{T,Ti};
    v11=one(T),
    v_diag_block::Union{Nothing,AbstractVector}=nothing,
    v_offdiag_block::Union{Nothing,AbstractVector}=nothing) where {T<:Real,Ti<:Integer}
    sdiag = _sbp6_experimental_extract_diagonal(S)
    return sbp6_experimental_vector_mass(
        sdiag;
        v11=v11,
...
"""
    sbp6_experimental_scalar_mass_gradient(source; accuracy_order=6, points=21, h=1,
...
"""
function sbp6_experimental_scalar_mass_gradient(source;
    ...
    Rq = _sbp6_experimental_as_big_rational(R)
...
@inline _sbp6_experimental_as_big_rational(x::Rational{BigInt}) = x
@inline _sbp6_experimental_as_big_rational(x::Integer) = big(x) // 1
@inline _sbp6_experimental_as_big_rational(x::Rational{<:Integer}) = big(numerator(x)) // big(denominator(x))

function _sbp6_experimental_as_big_rational(x::AbstractFloat)
...
function _sbp6_experimental_as_big_rational(x::Real)
    throw(ArgumentError("Could not convert value of type $(typeof(x)) to Rational{BigInt}."))
end

function _sbp6_experimental_maxabs(values)
...
function _sbp6_experimental_is_symmetric(A::SparseMatrixCSC{T,Ti}; tol::Float64=1e-12) where {T<:Real,Ti<:Integer}
    if T <: AbstractFloat
...
function _sbp6_experimental_is_pd(A::SparseMatrixCSC{T,Ti}) where {T<:Real,Ti<:Integer}
    n, m = size(A)
    n == m || return false
    try
...
"""
    sbp6_experimental_construct_divergence(S, V, Geven, r; p=2)
...
"""
function sbp6_experimental_construct_divergence(S::SparseMatrixCSC{T,Ti},
...
function _sbp6_experimental_constraint_rows(N::Int, closure_right::Int, first_rows_r5::Int)
...
function _sbp6_experimental_constraint_error(D::SparseMatrixCSC, r::AbstractVector, p::Int, degree::Int, rows::Vector{Int})
...
"""
    sbp6_experimental_solve_accuracy_constraints(setup; boundary_closure=nothing, first_rows_r5=5,
...
`setup` is the output of `sbp6_experimental_scalar_mass_gradient`.
"""
function sbp6_experimental_solve_accuracy_constraints(setup::NamedTuple;
    ...
    rows = _sbp6_experimental_constraint_rows(N, closure_right, first_rows_r5)

    tmpl = sbp6_experimental_vector_mass(N=N)
    ...
    Tq = Rational{BigInt}
    rq = Tq[_sbp6_experimental_as_big_rational(ri) for ri in r]
    I, J, GV = findnz(sparse(Geven))
    Gq = sparse(I, J, Tq[_sbp6_experimental_as_big_rational(gij) for gij in GV], N, N)
    Rq = _sbp6_experimental_as_big_rational(setup.R)
    ...
    residual_exact = m == 0 ? 0 // 1 : _sbp6_experimental_maxabs(abs.(A * x_exact .- bvec))
    ...
    S = spdiagm(0 => sdiag)
    V = sbp6_experimental_vector_mass(
        sdiag;
        v11=one(Tm),
        v_diag_block=v_diag_block,
        v_offdiag_block=v_offdiag_block
    )
    div_data = sbp6_experimental_construct_divergence(
        S,
        V,
        G_use,
        r_use;
    ...
    err_r = _sbp6_experimental_constraint_error(D, r_use, p, 1, rows.rows_r)
    err_r3 = _sbp6_experimental_constraint_error(D, r_use, p, 3, rows.rows_r3)
    err_r5 = _sbp6_experimental_constraint_error(D, r_use, p, 5, rows.rows_r5)
    quad_val = sum(sdiag)
    quad_target = Rq^(p + 1) / convert(Tq, p + 1)
    quad_err = Float64(abs(quad_val - quad_target))

    s_sym = _sbp6_experimental_is_symmetric(S)
    v_sym = _sbp6_experimental_is_symmetric(V)
    s_pd = all(si -> si > 0 // 1, sdiag)
    v_pd = _sbp6_experimental_is_pd(V)
...
"""
    sbp6_experimental_solve_accuracy_constraints(source; kwargs...)

Convenience overload that first builds folded SBP6 gradient/scalar-mass data using
`sbp6_experimental_scalar_mass_gradient` and then solves the linear split-mass constraints.
"""
function sbp6_experimental_solve_accuracy_constraints(source;
    accuracy_order::Int=6,
    points::Int=21,
    h::Real=1,
    N=points - 1,
    ...
    exact_solve::Bool=true,
    verbose::Bool=true)
    setup = sbp6_experimental_scalar_mass_gradient(
        source;
        accuracy_order=accuracy_order,
        points=points,
        h=h,
    ...
    return sbp6_experimental_solve_accuracy_constraints(
        setup;
...
"""
    sbp6_experimental_operators(source, points; kwargs...)
...
"""
function sbp6_experimental_operators(source,
    points::Integer;
    h::Real=1,
    accuracy_order::Int=6,
    p::Int=2,
    ...
    solved = sbp6_experimental_solve_accuracy_constraints(
        source;
        accuracy_order=accuracy_order,
        points=points_int,
        h=h,
