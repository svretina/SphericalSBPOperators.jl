...
function _compute_order_rows_diagonal(order::Int;
    ...
    for (idx, src) in enumerate(gathered.sources)
        ...
        try
            ops = _build_diagonal_ops(src, order; points = points, R = R, p = p, mode = mode)
...
function write_laplacian_divg_tables_diagonal(path::AbstractString, order_rows::Dict{Int, Any};
                                              orders::Tuple{Vararg{Int}},
                                              points::Int,
                                              p::Int,
                                              R::Rational{BigInt})
...
function generate_diagonal_tables(;
                                  ...
                                  mode = SafeMode(),
                                  out_hyper::AbstractString = joinpath("papers", "hyperbolic_sat_spectrum_tables_diagonal.tex"),
                                  out_lap::AbstractString = joinpath("papers", "laplacian_divg_spectrum_tables_diagonal.tex"),
    ...
    for order in orders
        ...
        results_by_order[order] = _compute_order_rows_diagonal(
            order;
            points = points,
            R = R,
            p = p,
    ...
    write_hyperbolic_tables_diagonal(
        out_hyper,
        results_by_order;
        orders = orders,
        points = points,
    ...
    write_laplacian_divg_tables_diagonal(
        out_lap,
        results_by_order;
        orders = orders,
        points = points,
