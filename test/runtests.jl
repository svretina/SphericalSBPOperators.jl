...
@testset "Staggered wave evolution path" begin
    ...
    @test size(sol.Ξ, 1) == length(ops.r)
    @test all(isfinite, sol.Π)
    @test all(isfinite, sol.Ξ)
...
@testset "Wave SAT boundary and symmetry tests" begin
    ...
    w_reflect_residual = sol_ref.Π[end, :] .+ sol_ref.Ξ[end, :] .-
        (sol_ref.Π[end, :] .- sol_ref.Ξ[end, :])
    ...
    @test maximum(abs.(sol_no_enforce.Ξ[1, :])) >= 1.0e-8
end
...
@testset "Wave initial-data modes and potential consistency" begin
    ...
    @test maximum(abs.(sol_pot.Ξ[:, 1] .- Ξ_expected)) <= 1.0e-10

    Π_bump = bumpb_profile(r; amplitude = 1.0, center = 0.0, radius = 0.35)
    Ξ_zero = zeros(Float64, length(r))
    pcheck = check_potential_consistency(ops, Π_bump, Ξ_zero; tol = 1.0e-10)
    @test pcheck.xi_growth_expected_from_pi
    @test pcheck.growth_explanation ==
        "Ξ appears immediately if Π0 varies because Ξ_t = G Π."
    ...
    @test maximum(abs.(char.Ξ0[2:end] .- 1.0)) <= 1.0e-12
    @test char.Ξ0[1] == 0.0
end
